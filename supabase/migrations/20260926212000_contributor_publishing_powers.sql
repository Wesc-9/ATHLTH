-- ATHLTH 1.4.1: Contributor publishing powers.
-- Contributor can create events/challenges, pin official updates and use
-- @everyone, while remaining unable to manage membership or delete groups.
...
create or replace function public.set_community_group_announcement_pin(
  p_group_id uuid,
...
begin
  if (select auth.uid()) is null
     or not private.can_publish_community_group_update(
       p_group_id
     )
  then
    raise exception 'Group publishing permission required';
  end if;

  update public.community_group_announcements
  set
    pinned_at = null,
    updated_at = now()
  ...
  if p_announcement_id is not null then
    ...
    if not found then
      raise exception 'Group update not found';
    end if;
  end if;
end;
$$;
...
begin
  ...
  sender_role := private.community_group_role(
    new.group_id,
    new.sender_id
  );
...
revoke all on function private.can_create_community_group_content(
  uuid,
  uuid
