-- ATHLTH Friends + Social + Challenge backend
-- Permanent schema migration. Safe to re-run against an environment where
-- the social foundation has already been provisioned.
...
create table if not exists public.profile_social_settings (
  ...
  allow_challenge_invites text not null default 'friends'
    check (allow_challenge_invites in ('friends','everyone','nobody')),
...
create table if not exists public.user_reports (
  ...
  reason text not null
    check (reason in ('spam','harassment','impersonation','unsafe_content','other')),
...
create table if not exists public.social_presence (
  ...
  state text not null default 'available'
    check (state in ('offline','available','training')),
...
create table if not exists public.social_activities (
  ...
  kind text not null
    check (kind in ('workout','trophy','goal','challenge','personal_record')),
...
create index if not exists social_activity_reactions_activity_idx
  on public.social_activity_reactions(activity_id);
create index if not exists social_activity_reactions_user_idx
  on public.social_activity_reactions(user_id);
...
create table if not exists public.social_challenges (
  ...
  status text not null
    check (status in ('draft','invited','upcoming','active','completed','cancelled')),
...
create index if not exists social_challenges_creator_idx
  on public.social_challenges(creator_id, created_at desc);
create index if not exists social_challenges_window_idx
  on public.social_challenges(starts_at, ends_at);
...
create table if not exists public.social_inbox_events (
  ...
  recipient_id uuid not null references public.profiles(id) on delete cascade,
  kind text not null
    check (kind in ('friend_request','friend_accepted','challenge_invite','challenge_result','reaction')),
  title text not null,
...
insert into public.profile_social_settings (
  user_id, profile_visibility, share_training_presence
)
select
  p.id,
...
as $$
  select
    private.can_view_social_profile(owner_id)
    and (
      (select auth.uid()) = owner_id
      or coalesce((
...
on public.profile_social_settings for select
to authenticated
...
on public.profile_social_settings for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

drop policy if exists social_profile_cards_select_allowed on public.social_profile_cards;
create policy social_profile_cards_select_allowed
on public.social_profile_cards for select
to authenticated
...
create policy social_performance_select_allowed
on public.social_performance_stats for select
to authenticated
...
create policy social_performance_insert_own
on public.social_performance_stats for insert
to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists social_performance_update_own on public.social_performance_stats;
create policy social_performance_update_own
on public.social_performance_stats for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

drop policy if exists social_trophies_select_allowed on public.social_trophy_showcases;
create policy social_trophies_select_allowed
on public.social_trophy_showcases for select
to authenticated
...
create policy social_trophies_insert_own
on public.social_trophy_showcases for insert
to authenticated
with check ((select auth.uid()) = user_id);

drop policy if exists social_trophies_update_own on public.social_trophy_showcases;
create policy social_trophies_update_own
on public.social_trophy_showcases for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

drop policy if exists social_presence_select_allowed on public.social_presence;
create policy social_presence_select_allowed
...
create policy social_activities_update_own
...
create policy social_activities_delete_own
...
on public.social_activity_reactions for select
to authenticated
using (
  exists (
    select 1
    ...
    where a.id = activity_id
      and private.can_view_activity(a.actor_id, a.visibility)
  )
);
...
on public.social_activity_reactions for insert
to authenticated
with check (
...
create policy social_reactions_update_own
on public.social_activity_reactions for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);
...
on public.social_activity_reactions for delete
to authenticated
using ((select auth.uid()) = user_id);
...
create policy social_challenges_insert_creator
...
create policy social_challenges_update_creator
...
create policy social_challenge_participants_select_allowed
on public.social_challenge_participants for select
to authenticated
...
create policy social_challenge_participants_insert_creator
on public.social_challenge_participants for insert
to authenticated
...
on public.social_challenge_participants for update
to authenticated
...
create policy social_challenge_attempts_select_allowed
on public.social_challenge_attempts for select
to authenticated
using (private.can_view_challenge(challenge_id));

drop policy if exists social_challenge_attempts_insert_self on public.social_challenge_attempts;
create policy social_challenge_attempts_insert_self
on public.social_challenge_attempts for insert
to authenticated
with check (
  (select auth.uid()) = user_id
  and exists (
...
create policy social_challenge_checkins_select_allowed
on public.social_challenge_checkins for select
to authenticated
using (private.can_view_challenge(challenge_id));

drop policy if exists social_challenge_checkins_insert_self on public.social_challenge_checkins;
create policy social_challenge_checkins_insert_self
on public.social_challenge_checkins for insert
to authenticated
with check (
  (select auth.uid()) = user_id
  and exists (
...
create policy social_challenge_checkins_update_self
on public.social_challenge_checkins for update
to authenticated
using ((select auth.uid()) = user_id)
with check ((select auth.uid()) = user_id);

drop policy if exists social_inbox_select_own on public.social_inbox_events;
create policy social_inbox_select_own
on public.social_inbox_events for select
to authenticated
using ((select auth.uid()) = recipient_id);

drop policy if exists social_inbox_update_own on public.social_inbox_events;
create policy social_inbox_update_own
on public.social_inbox_events for update
to authenticated
using ((select auth.uid()) = recipient_id)
with check ((select auth.uid()) = recipient_id);

drop policy if exists social_inbox_delete_own on public.social_inbox_events;
create policy social_inbox_delete_own
on public.social_inbox_events for delete
to authenticated
using ((select auth.uid()) = recipient_id);
...
begin
  if tg_op = 'INSERT' then
    if actor is null or actor <> new.sender_id then
      raise exception 'Friend request sender must match authenticated user';
    end if;
  ...
  if old.status <> 'pending' then
    raise exception 'Friend request has already been resolved';
  end if;
...
begin
  if tg_op = 'INSERT' then
    ...
    values (
      ...
      'friend_request',
      'New friend request',
      sender_name || ' wants to be friends on ATHLTH.',
      'friend_request',
      new.id
    );
  ...
  if old.status = 'pending' and new.status = 'accepted' then
    ...
    select coalesce(display_name, username::text, 'Your friend')
      into recipient_name
    from public.social_profile_cards
    ...
    values (
      ...
      recipient_name || ' accepted your friend request.',
      'friendship',
      new.id
    );
  end if;
...
create trigger friend_requests_after_change
...
create trigger social_challenge_participants_guard
before insert or update on public.social_challenge_participants
...
begin
  if old.rules_locked_at is not null or now() >= old.starts_at then
    ...
    if new.rules_locked_at is null then
      new.rules_locked_at := old.starts_at;
    end if;
  end if;
...
create trigger social_challenges_guard_update
...
after insert on public.social_challenge_participants
...
begin
  ...
  select
    ...
    new.participant_name || ' posted ' || new.detail || ' in ' ||
      coalesce(challenge_title, 'a challenge') || '.',
    'challenge',
    new.challenge_id
  from public.social_challenge_participants p
...
create trigger social_challenge_attempts_event
after insert on public.social_challenge_attempts
...
declare
  ...
  reactor_name text;
begin
  select a.actor_id into owner_id
  from public.social_activities a
  ...
  values (
    ...
    'reaction',
    'New reaction',
    reactor_name || ' reacted to your ATHLTH activity.',
    'activity',
    new.activity_id
  );
...
create trigger social_activity_reactions_event
after insert on public.social_activity_reactions
